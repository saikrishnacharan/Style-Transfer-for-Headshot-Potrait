...
addpath('./Image_Metamorphisis/aasthana_cvpr2013_code_version_2.0');
addpath('./grabcut');
addpath('./grabcut/bin_graphcuts');
...
title("Dense Correspondance Output");
...
%% Changing the style image by adding a small noise
...
% figure, imshow(img_style);
%% Mask Generation by calling masker function
if compute_masks == 1
...
%% Pyramid Creation and computing gain 
pyramid1 = laplacianstacks(img_orig, levels);
energy1 = local_energy(pyramid1, levels);

pyramid2 = laplacianstacks(img_style, levels);
energy2 =local_energy(pyramid2, levels);

gain = energy_gain(energy1, energy2, levels);

%% Used Robust Transfer and reconstruct image function 
final_pyramid = transfer_gain(pyramid1, pyramid2, gain, levels);
final_img = reconstructer(final_pyramid, levels);
