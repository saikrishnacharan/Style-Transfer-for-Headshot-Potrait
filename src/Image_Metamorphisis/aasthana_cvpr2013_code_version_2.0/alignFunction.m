function img = alignFunction(orig, style)
    ...
    % Warps given 2 images to the triangulation specified considering weightages as the second last parameter.
    %Here since its 0, we would warp the style image (p2) 
    %to the specified triangulation of the content image (p1) and then return that as the final morphed style image
...
%     title("New Style Image");
    
    figure, imshow(new_style_img);
    axis image;
    hold on;
    plot(new_points1(:,1),new_points1(:,2),'r.')
    title("Morphed Output")
    imwrite(new_style_img, 'Inputs/war.png');
    img = new_style_img;
